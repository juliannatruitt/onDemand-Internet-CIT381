...
while True:
    ...
    send_internet_request("TEST1") # unknown device_id
    time.sleep(3)
    print(check_internet_status())
    time.sleep(30)

    send_internet_request("device1") # high priority, internet should turn on
    time.sleep(145)
# test request from low priorty, high priority, and unknow device ID
# show how when high priority device comes in, they automatically get internet on
